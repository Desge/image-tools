    ...
    <div className="min-h-screen">
      ...
      <header className="mb-8">
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight" style={{ color: 'var(--text-primary)' }}>
          {t.converter.pageTitleFormat.replace('{from}', from.name).replace('{to}', to.name)}
        </h1>
      ...
      <section className="mb-12 max-w-2xl">
        <h2 className="text-xl font-semibold mb-4" style={{ color: 'var(--text-primary)' }}>
          {t.converter.aboutFormat.replace('{from}', from.name).replace('{to}', to.name)}
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="p-4 rounded-xl" style={{ backgroundColor: 'var(--bg-secondary)' }}>
            <h3 className="font-medium" style={{ color: 'var(--text-primary)' }}>{from.name}</h3>
            <p className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>{(t.converter.format as Record<string, string>)[from.id] || from.description}</p>
          </div>
          <div className="p-4 rounded-xl" style={{ backgroundColor: 'var(--bg-secondary)' }}>
            <h3 className="font-medium" style={{ color: 'var(--text-primary)' }}>{to.name}</h3>
            <p className="mt-1 text-sm" style={{ color: 'var(--text-secondary)' }}>{(t.converter.format as Record<string, string>)[to.id] || to.description}</p>
          </div>
        </div>
      </section>
